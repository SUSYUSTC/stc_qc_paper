...
def get_Rov(rhf, Cocc_guess, Cvir_guess, thres):
    begin = time.time()
    ...
    Uocc_local, Uvir_local, Uaux = orbopt.DFAll(Rov).run(builder, thres)
...
def get_Rov_mix(rhf, Cocc_guess, Cvir_guess, thres, factor=1.0, order=6):
    begin = time.time()
    ...
    Uocc_local, Uvir_local, Uaux = mixed_opt.run(builder, thres)
